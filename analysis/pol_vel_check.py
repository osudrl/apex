...
with torch.no_grad():
    ...
    for i in range(num_steps):
        ...
        for j in range(simrate):
            ...
            joint_vel[curr_ind, :] = np.concatenate([cassie_env.cassie_state.joint.velocity[0:2], cassie_env.cassie_state.joint.velocity[3:5]])
            mj_pos[curr_ind, :] = np.array(curr_qpos)[cassie_env.pos_idx]
            mj_vel[curr_ind, :] = np.array(curr_qvel)[cassie_env.vel_idx]
            mj_joint_vel[curr_ind, :] = np.array(curr_qvel)[[10, 11, 17, 18]]
            torque[curr_ind, :] = cassie_env.cassie_state.motor.torque[:]
            if curr_ind >= window_size:
                for k in range(10):
                    filter_vel[curr_ind, k] = vel_est(t, motor_pos[curr_ind-window_size:curr_ind, k], order=order)
...
for i in range(5):
    ax[0][i].plot(time[start_ind:end_ind], motor_pos[start_ind:end_ind, i], label="est")
    ax[0][i].plot(time[start_ind:end_ind], mj_pos[start_ind:end_ind, i], label="mj", c='C1')
...
for i in range(5):
    ...
    ax[0][i].plot(time[start_ind:end_ind], self_torque[start_ind:end_ind, i], label="self")
    ax[0][i].set_title("Left " + titles[i])

    ax[1][i].plot(time[start_ind:end_ind], torque[start_ind:end_ind, i+5], label="lib")
    ax[1][i].plot(time[start_ind:end_ind], self_torque[start_ind:end_ind, i+5], label="self")
    ax[1][i].set_title("Right " + titles[i])
    ax[1][i].set_xlabel("Time (sec)")
