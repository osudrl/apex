...
def speedmatch_footorient_reward(self):
    ...
    # straight_diff = 8*np.abs(qpos[1] - self.y_offset)
    straight_diff = np.abs(qpos[1])
    # if np.abs(qpos[1] - self.y_offset) < 0.1:
        # straight_diff = 0
    if straight_diff < 0.05:
        straight_diff = 0
    # if orient_diff < 5e-3:
    ...
    # else:
        # orient_diff *= 30
    if self.l_foot_orient < 5e-3:
        self.l_foot_orient = 0
    else:
        self.l_foot_orient *= 100
    if self.r_foot_orient < 5e-3:
        self.r_foot_orient = 0
    else:
        self.r_foot_orient *= 100

    reward = .4*np.exp(-forward_diff) + .15*np.exp(-orient_diff) \
                + .15*np.exp(-straight_diff) + .1*np.exp(-y_vel) \
...
def speedmatch_footorient_footheightvel_actpenalty_reward(self):
    qpos = np.copy(self.sim.qpos())
    qvel = np.copy(self.sim.qvel())
