...
class CassieEnv_filtervel(CassieEnv_clean):
    ...
    def vel_est(self, y):
        t = np.linspace(0, 0.0005*(self.vel_window - 1), self.vel_window)
        p = np.polyfit(t, y, self.vel_poly_order)
        x = t[-1]
        vel = 0
        for i in range(len(p)-1):
            vel += p[i]*x**(self.vel_poly_order-i-1)
        return vel
    ...
    def step_simulation(self, action):
        ...
        self.prev_mpos = np.roll(self.prev_mpos, -1, axis=0)
        self.prev_mpos[-1, :] = self.cassie_state.motor.position[:]
        curr_mvel = np.zeros(10)
        for i in range(10):
            curr_mvel[i] = self.vel_est(self.prev_mpos[:, i])
        curr_mpos = self.cassie_state.motor.position[:]
        self.u = pd_in_t()
        for i in range(5):
            ...
            self.u.leftLeg.motorPd.torque[i]  = self.P[i] * (target[i] - curr_mpos[i]) + self.D[i]*(-curr_mvel[i])
            self.u.rightLeg.motorPd.torque[i] = self.P[i] * (target[i+5] - curr_mpos[i+5]) + self.D[i]*(-curr_mvel[i+5])
    ...
    def step_sim_basic(self, action):
        ...
        self.prev_mpos = np.roll(self.prev_mpos, -1)
        self.prev_mpos[-1, :] = self.cassie_state.motor.position[:]
        for i in range(10):
            curr_mvel[i] = self.vel_est(self.prev_mpos[:, i])
        curr_mpos = self.cassie_state.motor.position[:]
    ...
    def reset(self):
        ...
        self.cassie_state = self.sim.step_pd(self.u)
        self.prev_mpos = np.stack([self.cassie_state.motor.position[:] for _ in range(self.vel_window)])
    ...
    def get_full_state(self):
        ...
        for i in range(10):
            curr_mvel[i] = self.vel_est(self.prev_mpos[:, i])
        # print("filter:", curr_mvel)
        # print("est:", self.cassie_state.motor.velocity[:])
        if self.joint_rand:
        ...
        robot_state = np.concatenate([
            ...
            curr_mvel,                                     # actuated joint velocities
