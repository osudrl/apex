...
class CassieEnv_speed_no_delta_noheight_noaccel:
    def __init__(self, traj, simrate=60, state_est=False):
        ...
        self.observation_space = np.zeros(38 + 1)
        if self.state_est:
            self.observation_space = np.zeros(40 + 1)       # Size for use with state est
        ...
        if self.state_est:
            self.clock_inds = [38, 39]
        else:
            # TODO: SET CORRECT CLOCK INDS FOR NON STATE EST
    ...
    def reset(self):
        ...
        self.com_vel_offset = 0.1*np.random.uniform(-0.1, 0.1, 2)
        self.lfoot_vel = np.zeros(3)
        self.rfoot_vel = np.zeros(3)
        self.l_foot_diff = 0
        self.r_foot_diff = 0
        ...
        if self.dynamics_rand:
            ...
            fric_noise = [np.random.uniform(0.3, 1.2), np.random.uniform(1e-4, 1e-2), np.random.uniform(5e-5, 5e-4)]
    ...
    def compute_reward(self):
        ...
        # reward = speedmatch_joint_reward(self)
    ...
    def get_full_state(self):
        ...
        joint_position = np.concatenate([self.cassie_state.joint.position[0:2], self.cassie_state.joint.position[3:5]])
        ...
        robot_state = np.concatenate([
            ...
            joint_position,                                     # unactuated joint positions (not including foot)
