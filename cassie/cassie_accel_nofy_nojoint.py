...
class CassieEnv_accel_nofy_nojoint(CassieEnv_clean):
    ...
    def set_up_state_space(self):
        ...
        base_mir_obs = np.array([ 2, 3, 0.1, 1, # Foot xz pos
                                    4, -5, 6, -7, # Pel quat
                                    -13, -14, 15, 16, 17, -8, -9, 10, 11, 12, # Motor pos
                                    18, -19, 20,    # Pel trans accel
                                    -21, 22, -23,   # Pel rot vel
                                    -29, -30, 31, 32, 33, -24, -25, 26, 27, 28])  # Motor vel
    ...
    def get_full_state(self):
        ...
        lfoot_xz = [self.cassie_state.leftFoot.position[0], self.cassie_state.leftFoot.position[2]]
        rfoot_xz = [self.cassie_state.rightFoot.position[0], self.cassie_state.rightFoot.position[2]]

        # Use state estimator
        robot_state = np.concatenate([
            lfoot_xz,     # left foot xz position
            rfoot_xz,     # right foot xz position
            new_orient,                                 # pelvis orientation
            motor_pos,                                     # actuated joint positions

            new_translationalAcceleleration,                       # pelvis translational accel
