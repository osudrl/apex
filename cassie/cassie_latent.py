...
class CassieEnv_latent:
    ...
    def get_full_state(self):
        ...
        # apply latent
        norm_data = np.concatenate([qpos[self.pos_index], qvel[self.vel_index]])
        norm_data = np.divide((norm_data - self.data_min), self.data_max)
        latent_mu, latent_std = self.latent_model.encode(torch.Tensor(norm_data))
        out_state = np.concatenate([latent_mu.detach().numpy(), ext_state])
        return out_state
