...
class CassieEnv_mjvel(CassieEnv_clean):
    ...
    def step_simulation(self, action):
        ...
        curr_mvel = np.array(self.sim.qvel())[self.vel_idx]
        for i in range(5):
    ...
    def step_sim_basic(self, action):
        ...
        self.u = pd_in_t()
        curr_mpos = self.cassie_state.motor.position[:]
        curr_mvel = np.array(self.sim.qpos())[self.vel_idx]
        for i in range(5):
            # TODO: move setting gains out of the loop?
            # maybe write a wrapper for pd_in_t ?
            self.u.leftLeg.motorPd.pGain[i]  = 0
    ...
    def get_full_state(self):
        ...
        robot_state = np.concatenate([
            ...
            qvel[self.vel_idx],                                     # actuated joint velocities
