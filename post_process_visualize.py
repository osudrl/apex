...
ax1.plot(time[:], trajectory[:, 0], label='left-hip-roll'  )
# ax1.plot(time[:], trajectory[:, 1], label='left-hip-yaw'   )
ax1.plot(time[:], trajectory[:, 2], label='left-hip-pitch' )
ax1.plot(time[:], trajectory[:, 3], label='left-knee'      )
# ax1.plot(time[:], motors[:, 4], label='left-foot'      )
ax1.plot(time[:], trajectory[:, 5], label='right-hip-roll' )
# ax1.plot(time[:], trajectory[:, 6], label='right-hip-yaw'  )
ax1.plot(time[:], trajectory[:, 7], label='right-hip-pitch')
ax1.plot(time[:], trajectory[:, 8], label='right-knee'     )
# ax1.plot(time[:], trajectory[:, 9], label='right-foot'     )
# ax1.set_xlabel('Time')
ax1.set_ylabel('Traj Motor Position [deg]')
ax1.legend(loc='upper left')
ax1.set_title('Traj Motor Position')


    # # measured torques
    # ax4 = plt.subplot(row,col,2, sharex=ax1)
    # ax4.plot(time, torques_mea[:, 0], label='left-hip-roll'  )
    # # ax4.plot(time, torques_mea[:, 1], label='left-hip-yaw'   )
    # ax4.plot(time, torques_mea[:, 2], label='left-hip-pitch' )
    # ax4.plot(time, torques_mea[:, 3], label='left-knee'      )
    # # ax4.plot(time, torques_mea[:, 4], label='left-foot'      )
    # ax4.plot(time, torques_mea[:, 5], label='right-hip-roll' )
    # # ax4.plot(time, torques_mea[:, 6], label='right-hip-yaw'  )
    # ax4.plot(time, torques_mea[:, 7], label='right-hip-pitch')
    # ax4.plot(time, torques_mea[:, 8], label='right-knee'     )
    # # ax4.plot(time, torques_mea[:, 9], label='right-foot'     )
    # # ax4.set_xlabel('Time')
    # ax4.set_ylabel('Measured Torques [Nm]')
    # ax4.legend(loc='upper left')
    # ax4.set_title('Measured Torques')

#Plot Joint Positions
ax2 = plt.subplot(row,col,3, sharex=ax1)
joints = np.rad2deg(joints)
ax2.plot(time, joints[:, 0], label='left-knee-spring'  )
# ax2.plot(time, joints[:, 1], label='left-tarsus')
# ax2.plot(time, joints[:, 2], label='left-foot'  )
ax2.plot(time, joints[:, 3], label='right-knee-spring'  )
# ax2.plot(time, joints[:, 4], label='right-tarsus')
# ax2.plot(time, joints[:, 5], label='right-foot'  )
# ax2.set_xlabel('Time')
ax2.set_ylabel('Joint Position [deg]')
ax2.legend(loc='upper left')
ax2.set_title('Joint Position')

    # # foot force
    # ax3 = plt.subplot(row,col,4, sharex=ax1)
    # # ax3.plot(time, ff_left[:, 0], label='left-X'  )
    # # ax3.plot(time, ff_left[:, 1], label='left-Y'  )
