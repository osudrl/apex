...
class Actor_tmp(nn.Module):
  def __init__(self, state_dim, output_dim, hidden_size=64):
    super(Policy_tmp, self).__init__()
...
class ReplayBuffer():
  def __init__(self, state_dim, action_dim, max_size, recurrent):
...
def run_experiment(args):
  ...
  actor = Actor_tmp(env.observation_space, env.action_space)
  critic = Critic_tmp(env.observation_space)

  algo = DDPG(actor, critic, 1.0, args.actor_lr, args.critic_lr, discount=args.discount, tau=args.tau)

  replay_buff = ReplayBuffer(args.timesteps, algo.recurrent)
